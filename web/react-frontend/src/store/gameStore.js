...
const useGameStore = create((set, get) => ({
  ...
  connectWebSocket: (sessionId, playerName, playerAvatar) => {
    ...
    const wsUrl = `${protocol}//${window.location.host}/ws?session=${sessionId}&name=${encodeURIComponent(playerName)}&avatar=${playerAvatar}`
  ...
  acquireCard: (cardIndex) => {
    ...
    // Check if can afford
    const canAfford = myPlayer?.resources && (

...
describe('Card Names Utility Functions', () => {
  ...
  describe('getCardImagePath', () => {
    test('should return correct path for valid card name', () => {
      expect(getCardImagePath('golem_0022')).toBe('/images/golem_0022.JPG')
      expect(getCardImagePath('mint_0002')).toBe('/images/mint_0002.JPG')
      expect(getCardImagePath('trade_0002_0020')).toBe('/images/trade_0002_0020.JPG')
    })
    
    test('should return default path for invalid card name', () => {
      expect(getCardImagePath(null)).toBe('/images/golem_bg.JPG')
      expect(getCardImagePath('')).toBe('/images/golem_bg.JPG')
      expect(getCardImagePath(undefined)).toBe('/images/golem_bg.JPG')
    })
  })
  ...
  describe('getCardRenderConfig', () => {
    ...
    test('should return image config when sprite is disabled', () => {
      ...
      expect(config.imagePath).toBe('/images/mint_0002.JPG')
    })
    ...
    test('should return image config for cards without sprite mapping', () => {
      ...
      if (config.mode === 'sprite') {
        ...
        expect(config.imagePath).toBe('/images/upgrade_2.JPG')
      }
    })
    ...
    test('should return default for invalid input', () => {
      ...
      expect(config.mode).toBe('image')
      expect(config.imagePath).toBe('/images/golem_bg.JPG')
    })
  })
  ...
  describe('getTokenRenderConfig', () => {
    ...
    test('should return image config when sprite is disabled', () => {
      ...
      expect(config.mode).toBe('image')
      expect(config.style).toBeNull()
      expect(config.imagePath).toBe('/images/stone_yellow.JPG')
    })
    ...
    test('should return image config for coins', () => {
      ...
      expect(config1.imagePath).toBe('/images/coin_3.JPG')
      
      const config2 = getTokenRenderConfig('silver_coin', { useSprite: false })
      expect(config2.imagePath).toBe('/images/coin_1.JPG')
    })
  })
